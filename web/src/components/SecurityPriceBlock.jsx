...
export default function SecurityPriceBlock({ listType, className, market = '', ...props }) {
    const dash = '\u2014'
    const sign = listType === exchangeTypes.bonds ? '%' : '\u20BD'
    return (
        <div className={`${className} security__price-block`}>
            <div title="Цена последней сделки" className="security__price">
                {props.price ? `${props.price} ${market !== '' && market === 'bonds' ? '%' : sign}` : dash}
            </div>
            {/* Should I extract this to another component? */}
            {
                (props.change > 0) &&
                <div title={changeTip} className="security__change-block">
                    <div className="security__change security__change_positive">
                        {props.change ? `+${props.change} ${market !== '' && market === 'bonds' ? '%' : sign}` : dash}
                    </div>
            ...
            }
            {
                (props.change < 0) && 
                <div title={changeTip} className="security__change-block">
                    <div className="security__change security__change_negative">
                        {props.change ? `${props.change} ${market !== '' && market === 'bonds' ? '%' : sign}` : dash}
                    </div>
            ...
            }
            {
                (props.change === 0) && 
                <div title={changeTip} className="security__change-block">
                    <div className="security__change">
                        {props.change ? `${props.change} ${market !== '' && market === 'bonds' ? '%' : sign}` : dash}
                    </div>
