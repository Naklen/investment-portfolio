    ...
    return (
        <div className="security-screen__trade-info">
            ...
            <div className="trade-info__info-unit offer-bid-unit">
                <div className="offer-bid-unit__offer" title={tips.offer}><span>Предложение:</span> { security.OFFER ? nf(security.OFFER) : '\u2014'}</div>
                <div className="offer-bid-unit__bid" title={tips.bid}><span>Спрос:</span> { security.BID ? nf(security.BID) : '\u2014'}</div>
            </div>
            <div className="trade-info__info-unit max-min-unit">
                <div className="max-min-unit__max" title={tips.max}><span>Максимум:</span> { security.HIGH ? nf(security.HIGH) : '\u2014'}</div>
                <div className="max-min-unit__min" title={tips.min}><span>Минимум:</span> { security.LOW ? nf(security.LOW) : '\u2014'}</div>
            </div>
            <div className="trade-info__info-unit counts-unit">
                <div className="counts-unit__trades-count" title={tips.count}><span>Сделок сегодня:</span> { security.NUMTRADES ? nf(security.NUMTRADES) : '\u2014'}</div>
                <div className="counts-unit__volume" title={tips.volume}><span>Количество сегодня:</span> { security.VOLTODAY ? nf(security.VOLTODAY) : '\u2014'}</div>
            </div>
            <div className="trade-info__info-unit capitalization-unit" title={tips.cap}><span>Капитализация:</span>
                {security.ISSUECAPITALIZATION ? nf(security.ISSUECAPITALIZATION) : '\u2014'}</div>
        </div>
    )
}
